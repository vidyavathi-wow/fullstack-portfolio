...
exports.getBook = (req, res) => {
    try {
        ...
        if (!book) return res.status(404).json({ success: false, message: "Book details do not match" });
...
exports.updateBook = (req, res) => {
    try {
        const { title, author, genre, published_year } = req.body;
        const stmt = db.prepare("UPDATE Books SET title = ?, author = ?, genre = ?, published_year = ? WHERE id = ?");
        const info = stmt.run(title, author, genre, published_year, req.params.id);
        if (info.changes === 0) return res.status(404).json({ success: false, message: "Book not found" });
