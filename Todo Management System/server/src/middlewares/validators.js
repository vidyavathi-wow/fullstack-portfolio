...
exports.validateTodo = [
  ...
  body('priority').optional().isIn(['Low','Moderate','High']).withMessage('Invalid priority'),
  body('status').optional().isIn(['pending','inProgress','completed']).withMessage('Invalid status'),
  body('category').optional().isIn(['Work','Personal','Other']).withMessage("Invalid category"),
